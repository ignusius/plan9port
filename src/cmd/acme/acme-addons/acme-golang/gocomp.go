...
func main() {
	...
	if f.offset0 == f.offset1 {

		out, err := exec.Command("gocode", "-in="+f.name, "autocomplete", strconv.Itoa(f.offset0)).Output()
		if err != nil {
			log.Fatal(err)
		}
		...
		args := strings.Split(string(f.body[f.offset0:f.offset1]), ".")
		if len(args) > 1 {
			out, err := exec.Command("godoc", args[0], args[1]).Output()
			if err != nil {
				log.Fatal(err)
			}
			fmt.Println(string(out))
		} else {
			out, err := exec.Command("godoc", args[0]).Output()
			if err != nil {
				log.Fatal(err)
			}
			fmt.Println(string(out))
